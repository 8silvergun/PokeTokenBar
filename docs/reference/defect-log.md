...
## 2026-09-18 — Evolution sprite cache miss could hold the Windows refresh gate

- **Symptom:** the evolution toast is delivered (for example Roselia), but immediately afterward the Windows app appears frozen: the new sprite does not load and later token refresh attempts do not recover it.
- **Root cause:** the serialized Windows `refreshing` gate covered remote sprite fetch, GIF decode, and all visual cache warming. Evolution is exactly when the active species changes to an uncached sprite, so a Schannel/raw.githubusercontent.com stall after the state mutation left `refresh()` waiting and every later `scheduleRefresh()` returned early because `refreshing == true`. The toast was not evidence that the refresh completed — it is posted synchronously from the already-committed lifecycle mutation.
- **Rule:** state/usage refresh must not wait for best-effort decoration. Publish the lifecycle snapshot first, invalidate the old visual subject by key, then load sprites in an independent task. Once detached, every completion must re-check the current visual key before it can publish; otherwise the fix trades a stall for a stale-completion race.
- **Windows network rule:** runtime sprite HTTPS follows the same bounded Schannel policy already used for PokéAPI — trusted system `curl.exe`, host/path allowlist, HTTPS-only redirects, best-effort revocation, hard connect/transfer/process deadlines, payload cap, and negative retry backoff. Never use `-k`/unbounded `URLSession.shared.data` on a path that can hold a product refresh gate.
- **Regression:** `WindowsSpriteNetworkingTests` pins the remote URL trust boundary; Windows CI continues to build/test/package the native product. Real-device verification is evolution on a cold sprite cache while temporarily breaking `raw.githubusercontent.com`: usage/popup refresh must continue even if the image stays on fallback.
