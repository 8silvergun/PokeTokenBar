...
- WSL 홈 경로 조회는 로그인 셸을 실행하지 않고 `printenv`만 호출합니다.
- 홈 경로의 `.`·`..`·제어문자와 배포판 이름의 경로 구분자를 거부합니다.
- 로그 재귀 스캔은 심볼릭 링크와 Windows reparse point를 따라가지 않습니다.
- WSL 명령의 임시 출력 파일은 이미 존재하는 파일을 덮어쓰지 않는 방식으로 생성합니다.

선택한 WSL 배포판 자체와 그 안의 로그 파일은 사용자가 신뢰하는 환경이어야 합니다.
