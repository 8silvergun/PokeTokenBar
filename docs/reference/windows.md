...
3. 시작 메뉴에서 PokeTokenBar를 실행합니다. 작업 표시줄의 숨겨진 아이콘 영역도 확인합니다.
4. 트레이 아이콘을 눌러 사용량과 캐릭터를 확인하고, 설정에서 로그인 시 자동 시작을 선택할 수 있습니다.
...
WSL의 Linux 홈 디렉터리는 Windows 홈과 다르며, 현재 빌드가 WSL 로그를 자동 검색한다고
가정해서는 안 됩니다. 별도 WSL 연동과 최신 macOS 전체 기능 동등성은 이 설치 검증의 범위가 아닙니다.
