...
final class WindowsPortTests: XCTestCase {
    ...
    func testRGBPacksBGROrder() {
        ...
        XCTAssertEqual(rgb(255, 0, 0), COLORREF(0x0000_00FF))
        XCTAssertEqual(rgb(0, 0, 255), COLORREF(0x00FF_0000))
    }
    ...
    func testWideIsNulTerminatedUTF16() {
        ...
        XCTAssertEqual("한".wide, [0xD55C, 0])
    }
    ...
    func testUpdateTagNormalize() {
        ...
        XCTAssertEqual(WindowsUpdate.normalize("win-2.4.4.1"), "2.4.4.1")
    }

    func testUpdateIsNewer() {
        XCTAssertTrue(WindowsUpdate.isNewer("2.4.5", than: "2.4.4"))
        XCTAssertTrue(WindowsUpdate.isNewer("2.4.10", than: "2.4.9"))
        XCTAssertTrue(WindowsUpdate.isNewer("2.5.0", than: "2.4.99"))
        XCTAssertFalse(WindowsUpdate.isNewer("2.4.4", than: "2.4.4"))
        XCTAssertFalse(WindowsUpdate.isNewer("2.4.3", than: "2.4.4"))
        XCTAssertTrue(WindowsUpdate.isNewer("2.4.4.1", than: "2.4.4"))
        XCTAssertTrue(WindowsUpdate.isNewer("2.4.4.2", than: "2.4.4.1"))
        XCTAssertTrue(WindowsUpdate.isNewer("2.4.5.0", than: "2.4.4.9"))
        XCTAssertFalse(WindowsUpdate.isNewer("2.4.4", than: "2.4.4.1"))
    ...
    func testLegacyGHUpdaterDownloadIsBlockedNarrowly() {
        XCTAssertTrue(WindowsProcess.isLegacyGHUpdateDownload(
            "gh release download 2.4.6 --repo 8silvergun/PokeTokenBar --pattern \"*Setup*.exe\" --dir C:\\tmp"))
        XCTAssertTrue(WindowsProcess.isLegacyGHUpdateDownload(
            "  GH   release   download 2.4.6 --repo 8SILVERGUN/PokeTokenBar --pattern x  "))
        XCTAssertFalse(WindowsProcess.isLegacyGHUpdateDownload(
            "gh release download 2.4.6 --repo another/repo --pattern x"))
        XCTAssertFalse(WindowsProcess.isLegacyGHUpdateDownload("gh auth status"))
        XCTAssertFalse(WindowsProcess.isLegacyGHUpdateDownload("codex app-server"))
    }
